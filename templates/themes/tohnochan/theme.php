...
class index
{
    public function build($action, $settings)
    {
        ...
        if ($action == "all") {
            copy(
                "templates/themes/tohnochan/" . $settings["basecss"],
                $config["dir"]["home"] . $settings["css"],
            );
        }
        ...
        if (
            ...
            $action = generation_strategy("sb_index", []);
            if ($action == "delete") {
                file_unlink($config["dir"]["home"] . $settings["html"]);
            } elseif ($action == "rebuild") {
                file_write(
                    $config["dir"]["home"] . $settings["html"],
                    $this->homepage($settings),
                );
            }
        }
    ...
    public function homepage($settings)
    {
        ...
        $query = preg_replace(
            '/UNION ALL $/',
            "ORDER BY `time` DESC LIMIT " . (int) $settings["limit_images"],
            $query,
        );
        ...
        if ($query == "") {
            error(
                _(
                    "Can't build the Index theme, because there are no boards to be fetched.",
                ),
            );
        }
        ...
        foreach ($boards as &$_board) {
            ...
            $query .= sprintf(
                "(SELECT *, '%s' AS `board` FROM ``posts_%s`` ORDER BY `time` DESC LIMIT %d) UNION ALL ",
                $_board["uri"],
                $_board["uri"],
                $limit,
            );
        ...
        while ($post = $query->fetch(PDO::FETCH_ASSOC)) {
            ...
            if ($post["thread"]) {
                ...
                $countQuery->execute();
                $post["snippet"] = $countQuery->fetchColumn() >= $config['noko50_min'] - 1;
        ...
        foreach ($boards as &$_board) {
            ...
            $query .= sprintf(
                "SELECT MAX(`id`) AS `top` FROM ``posts_%s`` UNION ALL ",
                $_board["uri"],
            );
        }
        ...
        foreach ($boards as &$_board) {
            ...
            $query .= sprintf(
                "SELECT `ip` FROM ``posts_%s`` UNION ALL ",
                $_board["uri"],
            );
        }
        $query = preg_replace('/UNION ALL $/', ") AS `posts_all`", $query);
        ...
        foreach ($boards as &$_board) {
            ...
            $query .= sprintf(
                "SELECT `files` FROM ``posts_%s`` UNION ALL ",
                $_board["uri"],
            );
        }
        $query = preg_replace(
            '/UNION ALL $/',
            " WHERE `num_files` > 0) AS `posts_all`",
            $query,
        );
        ($query = query($query)) or error(db_error());
