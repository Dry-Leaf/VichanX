...
function doBoardMapPart($list, $root, &$boards)
{
    global $config;
...
function doBoardListPart($list, $root, &$boards)
{
    ...
    foreach ($list as $key => $board) {
        if (is_array($board)) {
            $body .=
                ' <span class="sub" data-description="' .
                $key .
                '">[' .
                doBoardListPart($board, $root, $boards) .
                "]</span> ";
        } else {
            if (gettype($key) == "string") {
                if ($key == "category") {
                ...
                $title =
                    'title="' . $config["board_descriptions"][$board] . '"';
...
function error($message, $priority = true, $debug_stuff = false)
{
    ...
    die(
        Element($config["file_page_template"], [
            ...
            "body" => Element($config["file_error"], [
                "config" => $config,
                "message" => $message,
                "mod" => $mod,
                "board" => isset($board) ? $board : false,
                "debug" => $config["debug"]
                    ? (is_array($debug_stuff)
                        ? str_replace(
                            "\n",
                            "&#10;",
                            utf8tohtml(print_r($debug_stuff, true)),
                        )
                        : utf8tohtml($debug_stuff))
                    : null,
            ]),
        ])
    );
...
function loginForm($error = false, $username = false, $redirect = false)
{
    ...
    die(
        Element($config["file_page_template"], [
            ...
            "title" => _("Login"),
            "config" => $config,
            "body" => Element($config["file_login"], [
                "config" => $config,
                "error" => $error,
...
function truncate($body, $url, $max_lines = false, $max_chars = false)
{
    ...
    if ($body != $original_body) {
        ...
        $body .=
            ...
            sprintf(
                _(
                    'Post too long. Click <a href="%s">here</a> to view the full text.',
                ),
                $url,
            ) .
            "</span>";
...
function secure_link_confirm($text, $title, $confirm_message, $href)
{
    ...
    return '<a onclick="if (event.which==2) return true;if (confirm(\'' .
        htmlentities(addslashes($confirm_message)) .
        '\')) document.location=\'?/' .
        htmlspecialchars(
            addslashes($href . "/" . make_secure_link_token($href)),
        ) .
        '\';return false;" title="' .
        htmlentities($title) .
        '" href="?/' .
...
class Post
{
    public function __construct($post, $root = null, $mod = false)
    {
        ...
        if (isset($this->files) && $this->files) {
            $this->files = is_string($this->files)
                ? json_decode($this->files)
                : $this->files;
        ...
        if ($this->mod) {
            ...
            $this->body = preg_replace(
                '/<a((([a-zA-Z]+="[^"]+")|[a-zA-Z]+=[a-zA-Z]+|\s)*)href="' .
                    preg_quote($config["root"], "/") .
                    "(" .
                    sprintf(
                        preg_quote($config["board_path"], "/"),
                        $config["board_regex"],
                    ) .
                    ")/u",
                '<a $1href="?/$4',
    ...
    public function build($index = false)
    {
        ...
        return Element($config["file_post_reply"], [
            "config" => $config,
            "board" => $board,
            "post" => &$this,
            "index" => $index,
...
class Thread
{
    ...
    public function build($index = false, $isnoko50 = false)
    {
        ...
        $file =
            $index && $config["file_board"]
                ? $config["file_post_thread_fileboard"]
                : $config["file_post_thread"];
        $built = Element($file, [
            "config" => $config,
            "board" => $board,
            "post" => &$this,
